...
parser.add_argument('-p', '--print_freq', type=int, default=200,
...
save_dir = os.path.join('models_denoise', args.model + '_' + '1105vi_' + str(sigma))
...
if __name__ == '__main__':
    ...
    # from networks.DnCNN import DnCNN_R
    # model = DnCNN_R(1, 2, dep=17, num_filters=64, slope=0.2)

    from networks.UNet import UNet
    model = UNet(in_channels=1, out_channels=2, depth=4, wf=64, slope=0.2)

    # from networks.NestedUNet_V2 import NestedUNet_4
    # model = NestedUNet_4(input_channels=1, out_channels=2, slope=0.2)
    ...
    # scheduler = MultiStepLR(optimizer, milestones=[10, 20, 30, 40, 50], gamma=0.2)  # learning rates
    # optimizer
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    ...
    # train_im_list = generate_patch_from_mat(dir="./datasets/seismic/marmousi/marmousi20", pch_size=64, stride=[32,32])
    # train_im_list = train_im_list.astype(np.float32)

    # test_im_list = generate_patch_from_mat(dir="./datasets/seismic/marmousi", pch_size=128, stride=1024)
    # test_im_list = test_im_list.astype(np.float32)

    test_im_list = generate_patch_from_mat(dir="./datasets/seismic/overthrust", pch_size=128, stride=[24,64])
    test_im_list = test_im_list.astype(np.float32)
    ...
    # 'train': DenoisingDatasets_seismic_513.SimulateTrain(train_im_list, 5000 * args.batch_size,
    #                                                      args.patch_size, radius=args.radius,
    #                                                      noise_estimate=True),
    # DenoisingDatasets_seismic_513.SimulateTrain_Gauss(train_im_list, train_im_list.shape[0],
    #                                                   args.patch_size, radius=args.radius,
    #                                                   noise_estimate=True, sigma=75),

    # DenoisingDatasets_seismic_513.SimulateTrain_Gauss_NoisyLabel(train_im_list, train_im_list.shape[0],
    #                                                              args.patch_size, radius=args.radius,
    #                                                              noise_estimate=False, sigma=75, label_sigma=5),

    args.siesmic_dir = './datasets/seismic/marmousi/marmousi20'
    # train dataset
    path_SIDD_train = os.path.join(args.siesmic_dir, 'small_seismic_train_from_mat.hdf5')

    datasets = {'train': DenoisingDatasets_seismic.BenchmarkTrain_noaug(path_SIDD_train, 57834,
    ...
    for epoch in range(initial_epoch, n_epoch):
        ...
        loss_per_epoch = {x: 0 for x in ['Loss','lh', 'KLG']}
        ...
        # data_loader = {phase: torch.seis_utils.data.DataLoader(datasets[phase], batch_size=batch_size[phase],
        ...
        for n_count, batch_yx in enumerate(data_loader[phase]):
            ...
            loss, g_lh, kl_g = loss_fn_vi(phi_Z,im_noisy, im_gt, eps2=args.eps2, sigma0=75)
